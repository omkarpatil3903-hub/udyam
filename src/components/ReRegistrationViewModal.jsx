...
import { X, Printer, FileText, User, MapPin, CreditCard, Clock, CheckCircle, Hash } from 'lucide-react';

export default function ReRegistrationViewModal({ isOpen, onClose, data, onPaymentStatusUpdate, onWorkStatusUpdate }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[95vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 border border-gray-100">
                ...
                <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-bold text-gray-800">Re-registration Details</h2>
                ...
                <div className="flex-1 overflow-y-auto p-6 bg-white">
                    ...
                    {/* Status Banner */}
                    <div className="mb-6 flex items-center justify-between bg-purple-50 p-4 rounded-lg border border-purple-100">
                        <div className="flex gap-8">
                            <div>
                                <span className="text-xs font-bold text-purple-400 uppercase tracking-wider block mb-1">Payment Status</span>
                            ...
                            {isPaid && (
                                <div>
                                    <span className="text-xs font-bold text-purple-400 uppercase tracking-wider block mb-1">Work Status</span>
                            ...
                            <div>
                                <span className="text-xs font-bold text-purple-400 uppercase tracking-wider block mb-1">Applied On</span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-4">
                            <h3 className="text-sm font-bold text-gray-900 border-b pb-2 flex items-center gap-2">
                                <User size={16} className="text-purple-500" /> Applicant Information
                            </h3>
                        ...
                        <div className="space-y-4">
                            <h3 className="text-sm font-bold text-gray-900 border-b pb-2 flex items-center gap-2">
                                <Hash size={16} className="text-orange-500" /> Registration Details
                            </h3>
                            <div className="grid grid-cols-1 gap-y-3 text-sm">
                                <DetailRow label="UAM Number" value={data.uamNumber} />
                                <DetailRow label="State" value={data.state} />
                                <DetailRow label="Want Changes?" value={data.wantChanges ? 'Yes' : 'No'} />
                            </div>
                        </div>
                        ...
                        <div className="space-y-4 md:col-span-2">
                            <h3 className="text-sm font-bold text-gray-900 border-b pb-2 flex items-center gap-2">
                                <CheckCircle size={16} className="text-green-500" /> Agreements
                            </h3>
                            <div className="flex gap-6">
                                <div className="flex items-center gap-2">
                                    <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs ${data.agreedToTerms ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                                        {data.agreedToTerms ? '✓' : '✗'}
                                    </span>
                                    <span className="text-sm text-gray-700">Terms of Service</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs ${data.agreedToOTP ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                                        {data.agreedToOTP ? '✓' : '✗'}
                                    </span>
                                    <span className="text-sm text-gray-700">OTP & Additional Details</span>
                                </div>
                            </div>
                        </div>
