...
import { CreditCard, Clock, User, FileText, CheckCircle, Eye, AlertCircle, IndianRupee, Hash, XCircle } from 'lucide-react';
import CancelRegistrationViewModal from '../components/CancelRegistrationViewModal';
import AdminLayout from '../components/AdminLayout';

export default function CancelRegistrations({ onLogout }) {
    ...
    useEffect(() => {
        const q = query(collection(db, "cancelregistrations"), orderBy("timestamp", "desc"));
    ...
    const handlePaymentStatusUpdate = async (id, newStatus) => {
        try {
            if (!id) return;
            const registrationRef = doc(db, "cancelregistrations", id);
    ...
    const paidCompletedCount = registrations.filter(i => (i.paymentStatus === 'Paid' || i.paymentStatus === 'Successful') && i.workStatus === 'Completed').length;
        ...
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                <div
                    ...
                    className={`bg-white rounded-2xl p-5 shadow-sm border cursor-pointer hover:shadow-lg transition-all duration-300 group ${mainTab === 'paid' ? 'border-emerald-300 ring-2 ring-emerald-100' : 'border-gray-100'}`}
                >
                ...
                <div
                    ...
                    className={`bg-white rounded-2xl p-5 shadow-sm border cursor-pointer hover:shadow-lg transition-all duration-300 group ${mainTab === 'unpaid' ? 'border-orange-300 ring-2 ring-orange-100' : 'border-gray-100'}`}
                >
                    <div className="flex items-start justify-between">
                        <div>
                            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Unpaid</p>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                    <div className="flex items-center gap-3">
                        <h3 className="font-bold text-gray-900">Cancel Registrations</h3>
                        ...
                        {mainTab === 'paid' && (
                            <div className="flex bg-gray-100 rounded-lg p-0.5 ml-2">
                                <button
                                    ...
                                    className={`px-2 py-1 rounded text-[10px] font-semibold transition-all ${workFilter === 'all' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                                >
                                    All
                                </button>
                                <button
                                    onClick={() => setWorkFilter('pending')}
                                    className={`px-2 py-1 rounded text-[10px] font-semibold transition-all ${workFilter === 'pending' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                                >
                                    Pending ({paidPendingCount})
                                </button>
                                <button
                                    onClick={() => setWorkFilter('completed')}
                                    className={`px-2 py-1 rounded text-[10px] font-semibold transition-all ${workFilter === 'completed' ? 'bg-white text-emerald-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                                >
                ...
                <div className="overflow-x-auto">
                    <div className="min-w-[800px]">
                        ...
                        <div className="grid grid-cols-12 gap-4 px-6 py-3 bg-gray-50/50 text-xs font-bold text-gray-500 uppercase tracking-wider">
                            ...
                            <div className="col-span-2">Cancel Type</div>
                            ...
                            >
                                ...
                                <div className="col-span-2">
                                    <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold bg-red-100 text-red-700">
                                        <XCircle size={10} /> {item.cancellationType || 'N/A'}
                                    </span>
                                </div>
                                <div className="col-span-2">
                                    {mainTab === 'unpaid' ? (
                                        ...
                                        >
                                            <select
                                                ...
                                                onChange={(e) => handleWorkStatusUpdate(item.id, e.target.value)}
                                ...
                                <div className="col-span-2 flex justify-end gap-2">
                                    ...
                                    {mainTab === 'unpaid' && (
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                handlePaymentStatusUpdate(item.id, 'Paid');
                                            }}
                        ...
                        {filteredData.length === 0 && (
                            <div className="flex flex-col items-center justify-center py-16">
                                ...
                                <h3 className="text-lg font-bold text-gray-400">No cancel requests found</h3>
